...
const rendererCache = {};
const rendererCacheKeys = [];
const RENDERER_CACHE_MAX = 100;
...
async function getRenderer(width, height, version, format) {
  ...
  if (!rendererCache[key]) {
    if (rendererCacheKeys.length >= RENDERER_CACHE_MAX) {
      const evictKey = rendererCacheKeys.shift();
      delete rendererCache[evictKey];
    }
...
async function renderChartJsToWebp(
  width,
  height,
  backgroundColor,
  devicePixelRatio,
  ...
  const pngBuffer = await renderChartJs(
    width,
    height,
    backgroundColor,
    devicePixelRatio,
